...
static void fixLargeStructCallArgs(mlir::Operation *root) {
  ...
  for (auto callOp : calls) {
    ...
    for (auto &arg : newArgs) {
      ...
      auto ptrLoadOp = arg.getDefiningOp<mlir::ptr::LoadOp>();
      if (ptrLoadOp) {
        auto toPtrOp = ptrLoadOp.getPtr().getDefiningOp<mlir::ptr::ToPtrOp>();
        if (toPtrOp) {
          arg = toPtrOp.getPtr();
          newArgTypes.push_back(llvmPtrTy);
          changed = true;
          loadOpsToCheck.push_back(ptrLoadOp);
          continue;
        }
      }
      // Also try llvm.load pattern.
      auto llvmLoadOp = arg.getDefiningOp<mlir::LLVM::LoadOp>();
      if (llvmLoadOp) {
        arg = llvmLoadOp.getAddr();
        newArgTypes.push_back(llvmPtrTy);
        changed = true;
        loadOpsToCheck.push_back(llvmLoadOp);
        continue;
      }
      newArgTypes.push_back(arg.getType());
    }
    ...
    // Update external declarations' function types.
    if (fn.isExternal()) {
